...
export default {
  mounted() {
    var myMap = this.ZMap('container');
  },
  methods: {
    ZMap(id, center, level) {
      this.initCenter = this.ZPoint(116.404, 39.915); //初始化的中心点，同时为了定义网格的中心点
      this.id = id; //div的id
    ...
    initGrid() {
      ...
      for (
        var i =
          zMap.bounds.x1 +
          (zMap.initCenter.point.lng - zMap.bounds.x1) % span.x -
          span.x;
      ...
      for (
        var i =
          zMap.bounds.y1 +
          (zMap.initCenter.point.lat - zMap.bounds.y1) % span.y -
          span.y;
